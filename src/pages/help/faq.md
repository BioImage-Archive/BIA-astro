...
All publically available data in the BioImage Archive are accessible via [https://www.ebi.ac.uk/biostudies/BioImages/studies](https://www.ebi.ac.uk/biostudies/BioImages/studies). Data that are not yet public (e.g., with 'hold until published' status) can be accessed if you have the correct login credentials.
...
Accession numbers are assigned during the submission process, and do not change upon dataset publication or revision.

#### How long on average does it take to receive an accession number? 

Accession numbers are assigned after data are uploaded and the submission processed finalised. For small datasets, this can be a same day process, however for larger data volumes it will depend on transfer speeds (see above).

#### Do you assign Digital Object Identifiers (DOIs)? ####

Yes, DOIs are assigned automatically for new submissions. For old submissions, please [email us](mailto:bioimage-archive@ebi.ac.uk) to request a DOI.
...
#### How can I link to individual images?

Each image or file submitted to the BioImage Archive has an individual URI. The ~disnURIs keep to the following structure: `https://www.ebi.ac.uk/biostudies/files/<accession-id>/<file-path>` 
To learn the uri of a file in a submission, go to the webpage for that submission, then hover over the file or right click the file you want, and select “Copy Link Address”. 
...
#### Under what license(s) is the BioImage Archive data available?

Some accessions have individual licenses - these are explicitly stated on the page for that accession. Where no license is stated, data are available under the [EMBL-EBI Terms of Use](https://www.ebi.ac.uk/about/terms-of-use/) which places no restrictions on use or redistribution.

#### What image data formats do you support for downloading?

The BioImage Archive does not currently provide format transformations, so users can download data in the same formats as used by data depositors.
