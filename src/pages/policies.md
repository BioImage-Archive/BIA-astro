...
Version 1.2  
Last modified date: September 17 2026
...
2. Submitters are advised that the information displayed on the BioImage Archive and BioStudies websites is fully disclosed to the public. It is the responsibility of the submitters to ascertain that they have the right to submit the data and that no restrictions on data sharing apply.
...
4. The data depositor is responsible for making sure that consent has been granted by any study co-authors to deposit data on their behalf, and for sharing all communications regarding a deposition with their co-authors as required.
5. When a submission is made, a release date is set on which the dataset will automatically be made public. This date may be up to two years from the time of submission and can be changed after the submission is completed, for example, to align the data release with a publication.
6. BioImage Archive accession numbers are assigned during the submission process, and do not change upon dataset publication or revision.
7. Digital Object Identifiers (DOIs) are assigned automatically for new submissions. For older submissions, depositors can contact [bioimage-archive@ebi.ac.uk](mailto:bioimage-archive@ebi.ac.uk) to request a DOI.
8. The submission template includes fields for acknowledging funding associated with deposited data. For Horizon Europe grants, submitters should include the funding stream, project name, project acronym and grant PID.
9. All new data directly submitted to the BioImage Archive will be made available under a CC0 or CC-BY-4.0 licence, datasets brokered/imported from other resources may have other licenses (see below).

Some accessions in the BioImage Archive are available under licences other than CC0 or CC-BY-4.0. Where this is the case, the licence is indicated as an attribute on the dataset.

## Dataset removal by archive staff

Circumstances may arise in which the integrity, correctness, ownership or provenance of data is called into question. In such unusual circumstances, the BioImage Archive may decide to make an entry, in whole or in part, obsolete (moved out of the active archive, but still publicly accessible) or to remove it entirely from the public record. Examples of cases in which public removal might occur are: 
* Formal retraction of a publication describing the record.
* A substantive claim that the data was submitted without the necessary rights, permissions, or consents.

Such claims must be raised through formal contact between the depositor’s institution and EMBL-EBI. The BioImage Archive cannot control any downstream use of data that has already been distributed publicly by third parties, even after removal from the resource.
